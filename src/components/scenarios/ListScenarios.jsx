...
const ListScenarios = observer(({ data, removeScenarios, removeEmailPlaner }) => {
  // if (data) {
  return (
    <ul className="list_container">
      <li className="tr_clients_row">
        ...
        <div className="utilite_tr_scenarios_row title_table" style={{ justifyContent: "center" }}>
          Инструменты
        </div>
      </li>
      {data ? (
        data.map((s) =>
          (s.event === "popup" && stateScenarios.type_scenarios.popup) || (s.event === "чат-строка" && stateScenarios.type_scenarios.string) ? (
          ...
          stateScenarios.type_scenarios.planer ? (
            <li key={p.message_id} className="tr_clients_row ">
              ...
              <div className="utilite_tr_scenarios_row">
                <span onClick={() => removeEmailPlaner(p)} className="scenarios_tools_btn_remove_text">
                  Удалить
                </span>
              </div>
            </li>
